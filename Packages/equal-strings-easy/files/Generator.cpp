...
ostream &operator<<(ostream &other, const vector<pair<T, T>> &v)
{
    for (const auto &x : v)
        other << x.first + 1 << ' ' << x.second + 1<< '\n';
    return other;
}
//generate sting s consist of n digits
string gen_strings(int n)
{
    string s = "";
    for (int i = 0; i < n; i++)
    {
        s += to_string(gen_int(0, 9));
    }
    return s;
}
void Generate_tests()
{
    ...
    for (int i = 0; i < t; i++)
    {
        int sz = gen_int(1, 100000);
        // generate string consists of lowercase English letters and '?' with length n	
        string s = gen_string(sz);
        for(int i = 0; i < sz; i++){
            int z = gen_int(1, 10);
            if(z % 2)
                s[i] = '?';
        }
        string t = gen_string(sz);
        for(int i = 0; i < sz; i++){
            int z = gen_int(1, 10);
            if(z % 2)
                t[i] = '?';
