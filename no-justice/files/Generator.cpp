...
// #define SINGLE_TEST
int tests_count = 1000, sum_of_n = 2e5;
string gen_string_me(int len = 0)
{
    assert(len >= 0 && len <= 1e7);
    int cnta = gen_int(0, len);
    int cntb = gen_int(0, len - cnta);
    int cntc = len - cnta - cntb;
    string str(len, 'A');
    for (char &ch : str)
    {
        if (cnta)
...
{
    tests_count = 1;
    for (int t = 1; t <= tests_count; t++)
    {
        int n = gen_int(1, 10000000);
        cout << n << '\n';
        string s = gen_string_me(n);
        cout << s << '\n';
    }
}
